...
function SummaryBody({ summary }: { summary: ReceiptAiSummary }) {
  return (
    <div className="space-y-4">
      ...
      {summary.source === "mock" && summary.fallback_reason ? (
        <div className="space-y-2 rounded-md border border-amber-500/30 bg-amber-500/10 px-3 py-2 text-xs text-amber-700 dark:text-amber-400">
          <div className="flex flex-wrap items-center gap-x-3 gap-y-1">
            ...
            {summary.attempted_provider && (
              <span>
                Attempted provider:{" "}
                <code className="font-mono">{summary.attempted_provider}</code>
              </span>
            )}
            {summary.attempted_model && (
              <span>
                Attempted model:{" "}
                <code className="font-mono">{summary.attempted_model}</code>
              </span>
            )}
          </div>
        ...
        <p className="rounded-md border border-amber-500/30 bg-amber-500/10 px-3 py-2 text-xs text-amber-700 dark:text-amber-400">
          Using local mock summary because no AI provider is configured.
        </p>
      ) : null}
