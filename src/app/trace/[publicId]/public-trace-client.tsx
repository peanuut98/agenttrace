...
import type { Receipt, RiskFlag } from "@/types/receipt";
...
function ReportLayout({ state }: { state: Extract<LoadState, { kind: "ready" }> }) {
  const { run, project, steps, receipt } = state;
      ...
      <div className="rounded-lg border bg-muted/30 p-4">
        <div className="space-y-3">
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            {(aiSummary?.source === "ai" || aiSummary?.source === "claude_compatible") && (
...
function AIAuditReportBody({
  ...
  return (
    <div className="space-y-5">
      <div className="flex flex-wrap items-center gap-2">
        <Badge variant={summary.source === "ai" || summary.source === "claude_compatible" ? "default" : "outline"}>
  ...
  if (trust.isAiMock) {
    return (
      <NoticeCard
        ...
        description="This audit report was generated using local mock logic because no AI provider is configured. Configure a Claude-compatible API provider to enable live AI-generated audit reports."
      />
    );
  }
  return null;
...
function APIReadiness() {
  const rows: Array<{ key: string; description: string }> = [
    { key: "AI_PROVIDER", description: "Selects the AI provider (mock or claude_compatible)" },
    { key: "CLAUDE_COMPATIBLE_API_KEY", description: "Enables live AI audit report generation" },
    { key: "CLAUDE_COMPATIBLE_MODEL", description: "Selects the Claude model" },
    { key: "CLAUDE_COMPATIBLE_API_BASE", description: "Configures the API endpoint" },
    {
